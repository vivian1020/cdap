...
import co.cask.cdap.proto.metadata.lineage.MetadataSearchResponse;
...
public class MetadataHttpHandler extends AbstractHttpHandler {
    ...
    if (!sort.isEmpty()) {
      ...
      if (sortSplit.length != 2) {
        throw new BadRequestException("'sort' parameter should be a space separated string containing the field " +
                                        "('name' or 'create_time') and the sort order ('asc' or 'desc'). Found " +
                                        sort);
      }
      sortBy = sortSplit[0];
      sortOrder = sortSplit[1];
      if (!"name".equalsIgnoreCase(sortBy) && !"create_time".equalsIgnoreCase(sortBy)) {
        throw new BadRequestException("Sort field must be one of 'name' or 'create_time'. Found " + sortBy);
      }
  ...
  private TreeSet<MetadataSearchResultRecord> applySorting(Set<MetadataSearchResultRecord> original, String sortBy,
                                                       String sortOrder) throws BadRequestException {
    Comparator<MetadataSearchResultRecord> comparator = getComparator(sortBy, sortOrder);
    TreeSet<MetadataSearchResultRecord> result = new TreeSet<>(comparator);
    result.addAll(original);
    return result;
  }

  private Comparator<MetadataSearchResultRecord> getComparator(String sortBy,
                                                               String sortOrder) throws BadRequestException {
    switch (sortBy) {
      case "name":
        switch (sortOrder) {
          ...
          case "desc":
            return NAME_DESCENDING_COMPARATOR;
        }
      default:
